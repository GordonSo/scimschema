...
def test_meta_schema_name():
    mock_schema = {"id": "urn:ietf:params:scim:schemas:core2:2.0:User", "name": "User"}
...
def test_meta_schema_invalid_name():
    ...
    mock_schema = {"id": schema_id, "name": "_User"}
    assert_error = None
    try:
    ...
    pattern_invalid_str = re.compile(
        "Model schema id {} has property name which is expected to be a valid name".format(
            schema_id
        )
    )
